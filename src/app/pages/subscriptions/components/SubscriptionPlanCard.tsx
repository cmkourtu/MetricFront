...
const SubscriptionPlanCard: React.FC<SubscriptionPlanCardProps> = ({
  ...
  const { updateCurrentUserTrigger, setUpdateCurrentUserTrigger } = useAuth();
  ...
  const handleSelectSubscriptionPlan = async (subscriptionId: string) => {
    try {
      ...
      if (data) {
        window.open(data.latest_invoice.hosted_invoice_url, '_blank');
      }
    } catch (error: any) {
      console.log(error);
      setErrorText(
      ...
      setUpdateCurrentUserTrigger(!updateCurrentUserTrigger);
    }
  };
